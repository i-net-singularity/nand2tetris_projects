...
class CompilationEngine(object):
    ...
    def compile_let(self):
        ...
        symbol_kind=self.symbol_table.kind_of(symbol_name)

        symbol_index=self.symbol_table.index_of(symbol_name)
        self.output_terminal_and_advance('identifier')
        # ('[' expression ']')?
        ...
        # VMコード出力 ##################
        self.vm_writer.write_pop(segments[symbol_kind],symbol_index)
        ################################
    ...
    def compile_if(self):
        ...
        # VMコード出力 ##################
        self.vm_writer.write_if(f"IF_TRUE{self.if_count}")
        ################################
        ...
        self.vm_writer.write_label(f"IF_TRUE{self.if_count}")
        ################################
        self.if_count += 1
        ...
        if(self.jt.token_type == 'KEYWORD' and self.jt.current_token == 'else'):
            self.output_terminal_and_advance('keyword')
            self.output_terminal_and_advance('symbol')
            self.vm_writer.write_label(f"IF_FALSE{self.if_count}")
            self.if_count += 1
            self.compile_statements()
            self.output_terminal_and_advance('symbol')
            self.if_count -= 1
    ...
    def compile_while(self):
        ...
        # VMコード出力 ##################
        self.vm_writer.write_label(f"WHILE_EXP{self.while_count}")
        ################################
        ...
        self.vm_writer.write_if(f"WHILE_END{self.while_count}")
        self.while_count += 1
        ################################
        ...
        # VMコード出力 ##################
        self.while_count -= 1
        self.vm_writer.write_goto(f"WHILE_EXP{self.while_count}")
        self.vm_writer.write_label(f"WHILE_END{self.while_count}")
        ################################
    ...
    def compile_subroutine_call(self):
        ...
        # メソッド呼び出しの場合は、インスタンスをpushし、メソッド名にはクラス名を適用する
        #is_method_call = type not in (ST_INT, ST_CHAR, ST_BOOLEAN) and type is not None
        
        # もしサブルーチン名が現在のクラスのメソッドであるなら、`this` をプッシュ
    ...
    def compile_term(self):
        ...
        # varname | varName '[' expression ']' | subroutineCall
            ...
            # subroutineCall
                ...
                # VMコード出力
                print(f"_symbol_name_ = {name} kind={self.symbol_table.kind_of(name)} index={self.symbol_table.index_of(name)}")
                self.vm_writer.write_push(segments[self.symbol_table.kind_of(name)],self.symbol_table.index_of(name)) # ### VMコード出力
        else:
            self.output_terminal_and_advance('identifier')
